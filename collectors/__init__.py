...
class AppleHealthCollector:
    ...
    def save_webhook_data(self, data: Dict) -> str:
        ...
        # Resolve to absolute path and ensure it stays within export_path
        ...
        if 'data' in data:
            for record in data['data']:
                ...
                if 'HeartRate' in metric_type:
                    metrics['heart_rate'].append({'date': date, 'value': value, 'type': 'HeartRate'})
                elif 'HeartRateVariability' in metric_type:
                    metrics['hrv'].append({'date': date, 'value': value, 'type': 'HeartRateVariability'})
                elif 'SleepAnalysis' in metric_type:
                    metrics['sleep'].append({'date': date, 'value': value, 'type': 'SleepAnalysis'})
                elif 'StepCount' in metric_type:
                    metrics['steps'].append({'date': date, 'value': value, 'type': 'StepCount'})

        return metrics


class CrossFitLogger:
    """Manual CrossFit workout logger — delegates to BioDatabase"""
    ...
    def log_wod(self,
                ...
                rpe: int = None,
                notes: str = None) -> int:
        """Log a CrossFit workout via BioDatabase"""
        ...
        db = BioDatabase(self.db_path)
        return db.save_crossfit_workout(workout)
